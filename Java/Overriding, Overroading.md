...
* ###### 참고 : 생성자 오버로딩은 클래스에 대한 객체를 new 생성할 때, 초기값을 주는 방법을 다양화시키고 싶을 때.
 ...
 #####    SO, 일반적으로 public.
 #####     static 메모리에 할당가능하며, return 되는 값에 대한 반환 자료형을 명시하고 
 #####    필요에 따라 (자료형 매개변수) 외부에서 전달되는 값을 받아서 내부에서 사용할 수 있게 하는 방식의 코드 구현을 할 수 있다. 
 ##### 4 ) "외부에서 온 name을 객체 안의 필드 this.name에 기록해놓겠다." 그것의 반환형은 void고 이는 아무것도 채워져있지 않은 공터를 뜻한다. 그때 return은 생략할 수 있다. 
 <br>
