...
#### 인스턴스가 생성될 때 호출되는 '인스턴스 초기화 메서드', 인스턴스 변수의 초기화 작업에 사용되며, 인스턴스 생성 시에 실행되어야 할 작업을 위해서도 사용된다. 

#### :round_pushpin: 생성자의 조건 및 특징 
...
#### :round_pushpin: 생성자 생성 과정
 ##### 먼저 연산자 new에 의해서 메모리(heap)에 해당 클래스의 인스턴스가 생성된다. 그 후, 생성자가 호출되어 수행되고 연산자 new의 결과로 생성된 인스턴스의 주소가 반환되어 참조변수에 저장된다. 

<br>

#### :round_pushpin: this와 this()
* ##### this : 호출 시 클래스 이름 대신 사용하는 예약어, 인스턴스 자신을 가리키는 그 주소가 저장되어있는 참조변수를 지칭하며 모든 인스턴스 메서드에서 숨겨진 채로 존재한다.
